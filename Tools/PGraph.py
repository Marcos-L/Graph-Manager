...
class PGraph(ABC):
    ...
    def plot(self, vertex=None, edge=None, text=None, color=None, block=True, subplot=None):
        """
        Plot the graph
        ...
        if subplot is None:
            ...
            for c in range(self.nc):
                # for each component
                for node in self.component(c):
                    subplot.text(node.x, node.y, "  " + node.name, **text)
                    subplot.plot(node.x, node.y, color=color[c,:], **vertex)
                    for v in node.neighbours():
                        subplot.plot([node.x, v.x], [node.y, v.y], color=color[c,:], **edge)
    
            subplot.grid(True)
        if block:
            plt.show()

    def highlight_path(self, path, block=True, subplot=None,**kwargs):
        """
        ...
        if subplot is None:
            ...
            for i in range(len(path)):
                if i < len(path) - 1:
                    e = path[i].edgeto(path[i+1])
                    self.highlight_edge(e, subplot=subplot, **kwargs)
                self.highlight_vertex(path[i], subplot=subplot, **kwargs)

    def highlight_edge(self, edge, scale=1.5, color='r', subplot=None):
        """
        ...
        if subplot is None:
            ...
            p1 = edge.v1
            p2 = edge.v2
            subplot.plot([p1.x, p2.x], [p1.y, p2.y], color=color, linewidth=3 * scale)

    def highlight_vertex(self, node, scale=1.5, color='r', subplot=None):
        """
        ...
        if subplot is None:
            ...
            subplot.plot(node.x, node.y, 'o', color=color, markersize=12 * scale)
